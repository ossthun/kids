...
function writtenDate(date, lang) {
  const { day, month, year } = date;
  const monthName = monthNames[lang][month - 1];

  if (lang === "en") return `the ${day}. of ${monthName} ${year}`;
...
export default function DatesPage() {
  ...
  useEffect(() => {
    const correct = writtenDate(date, language);

    const wrong1 = writtenDate({ ...date, day: date.day === 28 ? 27 : date.day + 1 }, language);
    const wrong2 = writtenDate({ ...date, month: date.month === 12 ? 1 : date.month + 1 }, language);
    const wrong3 = writtenDate({ ...date, year: date.year + 1 }, language);

    setAnswers(shuffle([correct, wrong1, wrong2, wrong3]));
  }, [date, language]);
  ...
  return (
    <main style={styles.page}>
      <h1 style={styles.title}>{t.datesTitle}</h1>

      <p style={styles.subtitle}>{t.datesSubtitle}</p>
